...
with st.spinner("요리용 챗봇 온쿡이 가능한 요리를 생각 중이에요..."):
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": user_prompt}
    ]
    oncook_response = ask_gpt(messages, model="gpt-4o")
...
# ── 사용자 메뉴 선택 ────────────────────────────────
selected_menu = select_one_by_image("메뉴를 선택하세요", menu_imgs)
...
with col2:
    if st.button("요리 시작하기 ▶️"):
        # 선택한 메뉴 확인
        selected_menu_final = st.session_state.get("menu_selected")
        if not selected_menu_final:
            st.warning("메뉴를 먼저 선택해 주세요!")
            st.stop()

        # 세션에 최종 메뉴와 요리용 챗봇 온쿡 응답 저장
        st.session_state["menu"] = selected_menu_final
        st.session_state["oncook_response"] = oncook_response

        # 페이지 이동
        st.switch_page("3_만드는방법")
